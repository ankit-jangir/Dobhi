...
const formSchema = z.object({
  ...
  phone: z.string()
    ...
    .regex(/^[6-9]\d{9}$/, { message: 'Please enter a valid Indian phone number' }),
  password: z.string()
      ...
      <div className="lg:hidden relative h-56 sm:h-64 w-full flex-shrink-0 bg-gradient-to-br from-blue-600 via-blue-500 to-blue-600 rounded-b-[2.5rem] shadow-xl">
        <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center pt-4">
          ...
          <p className="text-sm sm:text-base text-white/90 font-medium max-w-sm px-4">
            Professional cleaning at your doorstep. Quality care for your clothes.
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-start justify-center px-4 py-0 sm:px-6 md:px-8 lg:p-8 bg-gradient-to-br from-gray-50 to-white overflow-y-auto flex-1 scrollbar-hide">
        <div className="w-full max-w-md lg:mt-0 -mt-12 sm:-mt-16 md:-mt-20 lg:py-8 pb-8">
          <div className="bg-white rounded-3xl shadow-2xl p-6 sm:p-7 md:p-8 border border-gray-100">
            ...
            <p className="text-xs sm:text-sm md:text-base text-gray-600 mb-4 sm:mb-5 md:mb-6 lg:mb-8">Sign up to get started with Dobhi</p>

            {/* Signup Form */}
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2.5 sm:space-y-3 md:space-y-3.5">
                {/* Full Name Field */}
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs sm:text-sm font-semibold text-blue-500">Full Name</FormLabel>
                      <FormControl>
                        <div className="relative">
                          <User className="absolute left-2.5 sm:left-3 top-1/2 transform -translate-y-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-400 z-10" />
                          <Input
                            type="text"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs sm:text-sm font-semibold text-blue-500">Email</FormLabel>
                      <FormControl>
                        <div className="relative">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs sm:text-sm font-semibold text-blue-500">Phone Number</FormLabel>
                      <FormControl>
                        <div className="relative">
                          <Phone className="absolute left-2.5 sm:left-3 top-1/2 transform -translate-y-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-400 z-10" />
                          <Input
                            type="tel"
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-start space-x-2 space-y-0 pt-1">
                      ...
                      <div className="space-y-1 leading-none pt-1">
                        <FormLabel className="text-[10px] sm:text-xs text-gray-600 font-normal cursor-pointer leading-tight">
                          I agree to the{' '}
                          <Link to="/terms" className="text-blue-500 hover:text-blue-600 font-semibold whitespace-nowrap">
                            Terms of Service
                          </Link>{' '}
                          and{' '}
                          <Link to="/privacy" className="text-blue-500 hover:text-blue-600 font-semibold whitespace-nowrap">
                            Privacy Policy
                          </Link>
                        </FormLabel>
                        <FormMessage className="text-xs" />
